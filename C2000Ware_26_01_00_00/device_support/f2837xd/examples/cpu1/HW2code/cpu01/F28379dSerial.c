...
#define SIZE_PYTHON_RXBUFFER 512
char rxBufferFromPython[SIZE_PYTHON_RXBUFFER];
int16_t rxIndexFromPython = 0;
uint16_t LED16bits = 0;
uint16_t setLEDsFlag = 0;
...
__interrupt void RXAINT_recv_ready(void)
{
    ...
    if (RXAdata & 0xC000) {
        ...
        if (RXAdata == '\n')
        {
            rxBufferFromPython[rxIndexFromPython] = '\0';
            rxIndexFromPython = 0;
            if (setLEDsFlag == 0) { //if previous flag has been processed then set flag
                setLEDsFlag = 1;
            }

        } else if (RXAdata != '\r') { // Ignore carriage return

            if (rxIndexFromPython < (SIZE_PYTHON_RXBUFFER - 1))
            {
                rxBufferFromPython[rxIndexFromPython] = RXAdata;
                rxIndexFromPython++;
            }
            else
            {
